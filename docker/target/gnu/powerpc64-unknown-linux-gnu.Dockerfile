...
ARG RUST_TARGET=powerpc64-unknown-linux-gnu

ARG CROSS_TOOLCHAIN=powerpc64-linux-gnu
ARG CROSS_TOOLCHAIN_PREFIX=${CROSS_TOOLCHAIN}-
ARG CROSS_SYSROOT=/usr/${CROSS_TOOLCHAIN}

ARG OPENSSL_COMBO=linux-ppc64

ARG GCC_PKGS="g++-powerpc64-linux-gnu libc6-dev-ppc64-cross"
...
ENV CARGO_TARGET_POWERPC64_UNKNOWN_LINUX_GNU_LINKER=${CROSS_TOOLCHAIN_PREFIX}gcc \
    ...
    RUST_TEST_THREADS=1 \
    PKG_CONFIG_ALLOW_CROSS_powerpc64_unknown_linux_gnu=true \
