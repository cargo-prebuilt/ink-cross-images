...
ARG RUST_TARGET=aarch64-unknown-openbsd
ARG OPENBSD_ARCH=arm64

ARG CROSS_TOOLCHAIN=aarch64-unknown-openbsd"$OPENBSD_MAJOR"
...
ENV CARGO_TARGET_AARCH64_UNKNOWN_OPENBSD_LINKER="$CROSS_TOOLCHAIN_PREFIX"clang \
    AR_aarch64_unknown_openbsd="$CROSS_TOOLCHAIN_PREFIX"ar \
    CC_aarch64_unknown_openbsd="$CROSS_TOOLCHAIN_PREFIX"clang \
    CXX_aarch64_unknown_openbsd="$CROSS_TOOLCHAIN_PREFIX"clang++ \
    CMAKE_TOOLCHAIN_FILE_aarch64_unknown_openbsd=/opt/toolchain.cmake \
    BINDGEN_EXTRA_CLANG_ARGS_aarch64_unknown_openbsd="--sysroot=$CROSS_SYSROOT" \
    RUST_TEST_THREADS=1 \
    PKG_CONFIG_ALLOW_CROSS_aarch64_unknown_openbsd=true \
